...
describe("Blog Server with Database Interaction", () => {
  ...
  describe("Should output all posts from the DB", () => {
    ...
    it("it should GET all records stored in the database table", (done) => {
      ...
      let queryString = `INSERT INTO posts ( title, content, posterId ) VALUES (?);
      INSERT INTO posts ( title, content, posterId ) VALUES (?);
      INSERT INTO posts ( title, content, posterId ) VALUES (?);`;

      //TODO - provide example values matching respective field types
      let queryArgs = [['1', 'one', 1], ['2', 'two', 2], ['3', 'three', 3]];
        ...
        chai
          ...
          .end((err, res) => {
            ...
            res.body.forEach((element) => {
              ...
              element.should.have.property("title").that.is.oneOf(["1", "2", "3"]);
            });
            done();
          });
      });
